...
namespace DragonCombatSimulator
{
    class Program
    {
        //Global variable declaration to store points for player and dragon
        static int playerHitPoints = 100;
        static  int dragonHitPoints = 200;
        //Global random number constructor
        ...
        //create function DragonAttack that calculates dragon's points from game
        static void DragonAtack()
        {
            //pick random number between 1 and 100 and store it in hitOrMiss
            int hitOrMiss = rng.Next(1, 101);
            //check a condition to see if the dragon's hit is less than or equal to 80
            if (hitOrMiss <= 80)
            {
